...
|---|---|---|
| 1 | owner가 물건 근처에 계속 있음 | `OWNER_REGISTERED`만 발생, `ABANDONED_OBJECT` 없음 |
| 2 | owner가 물건을 두고 화면 밖으로 나감 | `ABANDONED_OBJECT` 발생 |
| 3 | non-owner 접근 후 물건 사라짐/이동 | `SUSPICIOUS_APPROACH`, `THEFT_SUSPECTED` 발생 |
| 4 | non-owner가 잠깐 지나감 | `THEFT_SUSPECTED` 없음 |
| 5 | 물건이 잠깐 가려졌다가 재등장 | `THEFT_SUSPECTED` 없음 |

성공 기준은 5회 중 4회 이상 성공이며, 실패 케이스는 `MODEL_EXPERIMENTS.md`에 조명, 거리, 객체 크기, FPS와 함께 기록한다.
